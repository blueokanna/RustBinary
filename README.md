...
| `std::io` streams | Implemented | Reader/writer APIs with resource limits |
| `no_std` | Not implemented | Core serializer currently depends on `std::io` |
...
| `fingerprint` | Yes | Structural fingerprint frames; implies `derive` |
| `reflection` | Yes | Runtime-readable generated metadata; implies `derive` |
| `static-size` | Yes | Compile-time bounds; implies `derive` |
| `simd` | Yes | Runtime detection and hot-scan dispatch |
| `bit-packing` | No | Bit-level layouts; implies `derive` |
| `adaptive` | No | Adaptive strings/collections; implies `bit-packing`, `simd` |
